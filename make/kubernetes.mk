...
## app-config-secret: Create the Kubernetes secret named app-config
.PHONY: app-config-secret
app-config-secret: KUBECTL_DOCKER_ARGS += --tty --interactive
app-config-secret: $(KUBE_SECRET_DIR)/.env
	-$(KUBECTL) --namespace $(KUBE_APP_NAMESPACE) delete secret app-config > /dev/null 2>&1
	$(KUBECTL) --namespace $(KUBE_APP_NAMESPACE) create secret generic app-config --from-env-file $<


## app-config-secret: Create the Kubernetes secret named app-config
.PHONY: get-secrets
describe-secrets: $(KUBE_SECRET_DIR)/.env
	$(KUBECTL_NS) describe secret app-config
...
kube-list-namespaces: $(KUBECTL_DEPS)
	$(KUBECTL_NS) get namespaces
...
entrypoint-kubectl: KUBECTL_DOCKER_ARGS += --tty --interactive
